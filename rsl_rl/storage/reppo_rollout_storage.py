...
class ReppoRolloutStorage:
    """Official REPPO rollout storage."""
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        for _ in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                truncations_batch = self.truncations[:, start:stop]
    ...
    def _save_hidden_states(self, hidden_states: tuple[HiddenState, HiddenState]) -> None:
        ...
        if self.saved_hidden_state_a is None:
            self.saved_hidden_state_a = [
                torch.zeros(self.observations.shape[0], *hid.shape, device=self.device) for hid in hid_a
            ]
            self.saved_hidden_state_c = [
                torch.zeros(self.observations.shape[0], *hid.shape, device=self.device) for hid in hid_c
            ]

        for i in range(len(hid_a)):
            self.saved_hidden_state_a[i][self.step].copy_(hid_a[i])
            self.saved_hidden_state_c[i][self.step].copy_(hid_c[i])

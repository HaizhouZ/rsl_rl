...
from rsl_rl.networks import EmpiricalNormalization, MLP, TanhNormal
...
class ActorQ(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        if kwargs:
            print("ActorQ.__init__ got unexpected arguments, which will be ignored: " + str([key for key in kwargs]))
        super().__init__()

        self.obs_groups = obs_groups
        num_actor_obs = 0
        for obs_group in obs_groups["policy"]:
            assert len(obs[obs_group].shape) == 2, "The ActorQ module only supports 1D observations."
        ...
        self.critic_obs_dim = num_critic_obs - num_actions
    ...
    def evaluate(self, obs: TensorDict | torch.Tensor, act: torch.Tensor, *args, return_logits: bool = False) -> torch.Tensor:
